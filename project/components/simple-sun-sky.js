...
void main() {
    ...
    // was using interp, switched to norm.y
    color = mix(sunColor, color, smoothstep(0.009, 0.012, norm.y));
    color = mix(fogColor, color, smoothstep(0.0005, 0.05, norm.y));

    // Zoe - added this line to double the sky on the bottom

    color = mix(fogColor, color, smoothstep(0.05, 0.5, norm.y));
    color = mix(sunColor, color, smoothstep(0.005, 0.1, norm.y));
        ...
        if (data.fogColor.toUpperCase() === 'NONE') {
            ...
            this.material = new THREE.ShaderMaterial({
                uniforms: {
                    ...
                    darkColor: {value: new THREE.Color(data.darkColor)},
                    sunNormal: {value: sunPos.normalize()},
                    sunColor: {value: new THREE.Color(data.lightColor)},
